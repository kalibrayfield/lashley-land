...
if( isset( $_POST['action'] ) ) {
    ...
    //if (wp_verify_nonce($_POST['property_nonce'], 'submit_property')) { echo 'Nonce';

        $new_property = array(
            'post_type'	=> 'property'
        );

        if( $enable_paid_submission == 'per_listing' ) { echo 'per listing';
        ...
        // End per listing if
        } else if( $enable_paid_submission == 'membership' ) { echo 'Membership';

            if ( !is_user_logged_in() ) { echo 'not logged innnnnnnnnnnnnnnnnnnn';
                $email = wp_kses( $_POST['user_email'], $allowed_html );
                if( email_exists( $email ) ) {
                    $errors[] = $houzez_local['email_already_registerd'];
                }
            ...
            } else {  echo 'logged innnnnnnnnnnnnnnnnnnn';
                $property_id = apply_filters('houzez_submit_listing', $new_property);
                $args = array(
                    'listing_title'  =>  get_the_title($property_id),
                    'listing_id'     =>  $property_id
